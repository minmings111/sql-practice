...
| 테이블 연결 | [17](./17_orders_completed_with_customer_name.sql), [18](./18_customers_completed_order_total_having.sql), [20](./20_order_items_active_category_sales.sql), [21](./21_orders_completed_total_paid_discount.sql), [22](./22_orders_completed_total_charged_tax.sql), [26](./26_couriers_delivery_counts_join.sql), [27](./27_customers_order_amounts_join.sql), [28](./28_teams_match_points_ranking.sql), [29](./29_players_game_points_ranking.sql), [30](./30_employees_review_scores_ranking.sql), [33](./33_players_score_trend_ranking.sql) |
| 조건부 집계 | [19](./19_events_latest_value_difference.sql), [23](./23_orders_status_amount_by_customer.sql), [24](./24_orders_completed_count_by_customer.sql), [25](./25_deliveries_late_count_by_courier.sql), [26](./26_couriers_delivery_counts_join.sql), [27](./27_customers_order_amounts_join.sql), [28](./28_teams_match_points_ranking.sql), [29](./29_players_game_points_ranking.sql), [30](./30_employees_review_scores_ranking.sql) |
| `LEFT JOIN`과 `COALESCE` | [28](./28_teams_match_points_ranking.sql), [29](./29_players_game_points_ranking.sql), [30](./30_employees_review_scores_ranking.sql), [33](./33_players_score_trend_ranking.sql) |
| `UNION ALL` | [28](./28_teams_match_points_ranking.sql), [29](./29_players_game_points_ranking.sql), [30](./30_employees_review_scores_ranking.sql) |
| 윈도우 함수 | [19](./19_events_latest_value_difference.sql), [31](./31_price_logs_latest_price_change.sql), [32](./32_sensor_readings_latest_delta.sql), [33](./33_players_score_trend_ranking.sql) |
...
`COUNT(*)`의 `*`는 행 전체를 세겠다는 뜻입니다. `CASE WHEN` 안에서는 `THEN (*)`처럼 쓰지 않고, 조건을 만족하는 행에 대해 `THEN 1`처럼 값을 반환합니다.
...
`JOIN` 뒤에 집계를 할 때, `SELECT`에 나온 일반 컬럼은 `GROUP BY`에도 포함합니다.
...
조건이 여러 개라면 `CASE WHEN` 안에서 `AND`로 연결합니다.

```sql
COUNT(CASE WHEN condition_a AND condition_b THEN 1 END)
```
